-- ============================================
-- SAN Contractor — Full Reset + Recreate
-- Run this in Supabase SQL Editor to wipe and
-- rebuild the schema from scratch cleanly.
-- ============================================

-- STEP 1: Drop everything in reverse dependency order
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP TRIGGER IF EXISTS set_updated_at_profiles ON profiles;
DROP TRIGGER IF EXISTS set_updated_at_boq_items ON boq_items;
DROP FUNCTION IF EXISTS handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS update_updated_at() CASCADE;

DROP VIEW IF EXISTS material_receipt_totals;

DROP TABLE IF EXISTS activity_log          CASCADE;
DROP TABLE IF EXISTS micro_vos             CASCADE;
DROP TABLE IF EXISTS mtn_requests          CASCADE;
...
-- Drop storage bucket policies if they exist
...
CREATE TABLE profiles (
  ...
  role       TEXT NOT NULL DEFAULT 'supervisor'
               CHECK (role IN ('supervisor', 'estimator', 'principal')),
...
CREATE POLICY "Project members can update boq_items" ON boq_items
  FOR UPDATE USING (
    EXISTS (SELECT 1 FROM project_assignments pa
            WHERE pa.project_id = boq_items.project_id AND pa.user_id = auth.uid())
  );
...
CREATE TABLE milestones (
  ...
  status       TEXT NOT NULL DEFAULT 'ON_TRACK'
                 CHECK (status IN ('ON_TRACK','AT_RISK','DELAYED','COMPLETE'))
);
...
CREATE TABLE material_requests (
  ...
  status       TEXT NOT NULL DEFAULT 'PENDING'
                 CHECK (status IN ('PENDING','APPROVED','REJECTED','AUTO_HOLD')),
  created_at   TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE purchase_orders (
  ...
  flag         TEXT NOT NULL DEFAULT 'OK'
                 CHECK (flag IN ('OK','INFO','WARNING','CRITICAL')),
  status       TEXT NOT NULL DEFAULT 'PENDING'
...
CREATE TABLE receipt_photos (
  ...
  photo_type   TEXT NOT NULL
                 CHECK (photo_type IN ('surat_jalan','material_site','vehicle','tiket_timbang')),
...
CREATE POLICY "Supervisors can insert receipt photos" ON receipt_photos
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM material_receipts mr
      WHERE mr.id = receipt_photos.receipt_id AND mr.received_by = auth.uid()
    )
  );
...
CREATE TABLE progress_reports (
  ...
  work_status      TEXT NOT NULL
                     CHECK (work_status IN ('IN_PROGRESS','COMPLETE','COMPLETE_DEFECT')),
  location         TEXT,
...
CREATE TABLE defects (
  ...
  status            TEXT NOT NULL DEFAULT 'OPEN'
                      CHECK (status IN ('OPEN','IN REPAIR','RESOLVED','VERIFIED')),
  photo_path        TEXT,
...
CREATE TABLE activity_log (
  ...
  type       TEXT NOT NULL
               CHECK (type IN ('progres','terima','permintaan','cacat','mtn','micro_vo','attendance')),
  label      TEXT NOT NULL,
  flag       TEXT NOT NULL DEFAULT 'OK'
               CHECK (flag IN ('OK','INFO','WARNING','CRITICAL')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
