...
__export(excelParser_exports, {
  BOQ_WORK_TYPES: () => BOQ_WORK_TYPES,
  applyBoqGrouping: () => applyBoqGrouping,
  autoMaterialCode: () => autoMaterialCode,
  convertToStagingRows: () => convertToStagingRows,
  disambiguateBoqLabels: () => disambiguateBoqLabels,
  getWorkTypeLabel: () => getWorkTypeLabel,
  groupBoqItems: () => groupBoqItems,
  normalizeConcreteGrade: () => normalizeConcreteGrade,
  parseBoqWorkbook: () => parseBoqWorkbook,
  reconcileMaterials: () => reconcileMaterials
});
module.exports = __toCommonJS(excelParser_exports);
var XLSX = __toESM(require("xlsx"));

// ../../../../../private/tmp/empty.js
var empty_default = {};
...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  console.error(
    "[SAN] Missing EXPO_PUBLIC_SUPABASE_URL or EXPO_PUBLIC_SUPABASE_ANON_KEY. Copy tmp/.env.example to .env and fill in your credentials."
  );
}
var supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
  auth: {
    storage: empty_default,
    autoRefreshToken: true,
    persistSession: true,
    detectSessionInUrl: false
  }
...
var COEFFICIENT_RULES = [
  ...
  { title: /beton.*1\s*:\s*2\s*:\s*3|site mix.*1\s*:\s*2\s*:\s*3/i, material: /semen/i, min: 6, max: 9.5, unit: "zak/m3" },
  ...
  { title: /(beton.*)?1\s*:\s*3\s*:\s*5|lantai kerja/i, material: /semen/i, min: 4, max: 6.5, unit: "zak/m3" },
  ...
  { title: /beton|cor\b|kolom|balok|plat|sloof|poer/i, material: /ready ?mix/i, min: 0.98, max: 1.1, unit: "m3/m3" },
...
var FORMWORK_RATIO_RANGES = {
  kolom: { min: 5, max: 9 },
  balok: { min: 5.5, max: 10 },
  plat: { min: 3, max: 6 },
  sloof: { min: 4, max: 8 },
  poer: { min: 3, max: 6 },
  tangga: { min: 6, max: 12 }
};
var REBAR_RATIO_RANGES = {
...
var WASTE_FACTOR_RANGE = { min: 0.02, max: 0.25 };
function parseBoqWorkbook(fileInput, fileName, options) {
  const workbook = typeof fileInput === "string" ? XLSX.readFile(fileInput, { cellFormula: true, cellNF: true }) : XLSX.read(fileInput, { cellFormula: true, cellNF: true });
  const sheetNames = workbook.SheetNames;
  const rabSheets = sheetNames.filter(
    (n) => /^RAB\s*\(/i.test(n) || /^RAB$/i.test(n)
  );
  ...
  const anomalies = [];
  const materials = materialSheet ? parseMaterialSheet(workbook.Sheets[materialSheet], materialSheet) : [];
  const laborRates = upahSheet ? parseUpahSheet(workbook.Sheets[upahSheet], upahSheet) : [];
  const materialPriceMap = buildMaterialPriceMap(materials);
  const laborPriceMap = buildLaborPriceMap(laborRates);
  const ahsBlocks = [];
  const ahsRowMap = /* @__PURE__ */ new Map();
  if (ahsSheet) {
  ...
  const markupFactors = rekapSheet ? parseRekapMarkup(workbook.Sheets[rekapSheet]) : [];
  const boqItems = [];
  const rabToAhsLinks = /* @__PURE__ */ new Map();
...
function disambiguateBoqLabels(items) {
  ...
  for (const group of groups.values()) {
    ...
    const uniqueContexts = new Set(
      group.map((item) => normalize(getBoqDisambiguationContext(item) ?? "")).filter(Boolean)
    );
    const usedLabels = /* @__PURE__ */ new Set();
    const contextCounts = /* @__PURE__ */ new Map();
    for (const item of group) {
      const baseContext = getBoqDisambiguationContext(item) ?? (item.elementCode ? `Elemen ${item.elementCode}` : `Baris ${item.sourceRow}`);
      ...
      if (uniqueContexts.size <= 1) {
        finalContext = item.elementCode ? `${baseContext} \xB7 ${item.elementCode}` : `${baseContext} \xB7 ${item.code}`;
      } else if (seenCount > 1) {
        finalContext = item.elementCode ? `${baseContext} \xB7 ${item.elementCode}` : `${baseContext} \xB7 ${item.code}`;
      }
      ...
      while (usedLabels.has(normalize(candidate))) {
        candidate = `${item.label} \u2014 ${finalContext} \xB7 ${serial}`;
        serial++;
      }
...
function getBoqDisambiguationContext(item) {
  const candidates = [item.sectionLabel, item.chapter, item.sourceSheet].filter(Boolean);
...
function extractFloorContext(raw) {
...
function formatFloorToken(token) {
  const cleaned = token.replace(/[^\da-z]/gi, "").trim();
  if (!cleaned) return token.trim();
  if (/^\d+$/.test(cleaned)) return cleaned;
  return cleaned.toUpperCase();
}
function compactSectionContext(raw) {
  return raw.replace(/^pekerjaan\s+/i, "").replace(/^fisik\s+/i, "").replace(/^struktur\s+/i, "").replace(/^arsitektur\s+/i, "").replace(/^mekanikal\s+elektrikal\s+/i, "").replace(/\s+/g, " ").trim();
}
function parseMaterialSheet(sheet, sheetName) {
  const results = [];
  ...
  for (let r = headerRow + 1; r <= range.e.r; r++) {
    ...
    const specCell = colMap.spec >= 0 ? sheet[XLSX.utils.encode_cell({ r, c: colMap.spec })] : null;
...
function dedupeByNameUnit(rows) {
...
function normalizeConcreteGrade(name) {
...
function detectMaterialColumns(sheet, headerRow, maxCol) {
  let name = 1;
  let spec = -1;
  let unit = 5;
  let price = 6;
...
function parseUpahSheet(sheet, sheetName) {
  const results = [];
  const range = XLSX.utils.decode_range(sheet["!ref"] ?? "A1");
  let headerRow = -1;
  for (let r = 0; r <= Math.min(10, range.e.r); r++) {
...
function parseAnalisaSheet(sheet, sheetName, materialPriceMap, laborPriceMap, anomalies) {
  const blocks = [];
  const range = XLSX.utils.decode_range(sheet["!ref"] ?? "A1");
      ...
      if (typeof bVal === "string" && bVal.trim().length > 5 && isAhsTitleRow(bVal)) {
        if (currentBlock && currentBlock.components.length > 0) {
          blocks.push({
            title: currentBlock.title,
            titleRow: currentBlock.titleRow + 1,
            jumlahRow: currentBlock.titleRow + 1,
            // no jumlah found
            components: currentBlock.components,
...
function isAhsTitleRow(text) {
  return /^\d+\s*(m[123]|kg|ls|bh|pcs|titik|set|unit)\s+/i.test(text.trim()) || /^(pekerjaan|pasangan|pemasangan|pengecoran|pembetonan|pembesian)/i.test(text.trim());
}
...
var ROMAN_RE = /^(I{1,3}|IV|VI{0,3}|IX|X{0,3}I{0,3}|X{0,3}V?I{0,3})\.?\s*$/;
function parseRabSheet(sheet, sheetName, ahsRowMap, ahsBlocks, sortOrderOffset, anomalies) {
  const items = [];
  ...
  for (let r = headerRow + 1; r <= range.e.r; r++) {
    ...
    const elementCodeCell = colMap.elementCode >= 0 ? sheet[XLSX.utils.encode_cell({ r, c: colMap.elementCode })] : null;
    ...
    items.push({
      ...
      compositeFactors: hasComposite ? { formwork_ratio: formworkRatio, rebar_ratio: rebarRatio, wiremesh_ratio: wiremeshRatio } : null,
      sourceRow: r + 1,
      sourceSheet: sheetName,
...
function detectRabColumns(sheet, headerRow, maxCol) {
  const defaults = {
    no: 0,
    // A
    uraian: 1,
    ...
    // C (SAT)
    volume: 7,
    // H (actual volume, source of truth)
    clientUnitPrice: 4,
    // E
    clientTotal: 5,
    // F
...
function traceAhsReferences(sheet, row, colMap) {
  const refs = [];
...
function addRefIfFormula(sheet, row, col, component, refs) {
  if (col < 0) return;
  const cell = sheet[XLSX.utils.encode_cell({ r: row, c: col })];
...
function resolveAhsBlockIndices(refs, ahsRowMap) {
...
function reconcileMaterials(excelMaterials, catalog, aliases) {
  ...
  for (const mat of excelMaterials) {
    ...
    if (exact) {
      resolved.push({ ...mat, resolvedCode: exact.code, matchConfidence: 1 });
      continue;
    }
...
function autoMaterialCode(name, spec) {
  const slug = (s) => s.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toUpperCase().replace(/[^A-Z0-9]+/g, "-").replace(/^-+|-+$/g, "");
...
var NUMERIC_TOKEN = /^\d+([.,]\d+)?$/;
var FUZZY_THRESHOLD = 0.8;
function extractDimensionTokens(tokens) {
...
function fuzzyMatch(query, catalog) {
  ...
  for (const entry of catalog) {
    ...
    const allTokens = /* @__PURE__ */ new Set([...queryTokenArr, ...entryTokenArr]);
...
function runAnomalyChecks(boqItems, ahsBlocks, materials, anomalies) {
...
function checkCoefficientDeviation(block, comp, anomalies) {
  const rule = COEFFICIENT_RULES.find((r) => r.title.test(block.title) && r.material.test(comp.description));
  if (!rule) return;
  const baseCoeff = comp.wasteFactor > 0 ? comp.coefficient / (1 + comp.wasteFactor) : comp.coefficient;
  if (baseCoeff >= rule.min * 0.5 && baseCoeff <= rule.max * 2) return;
  anomalies.push({
    type: "coefficient_deviation",
    severity: baseCoeff < rule.min * 0.3 || baseCoeff > rule.max * 3 ? "HIGH" : "WARNING",
...
function checkWasteFactor(block, comp, anomalies) {
...
function checkMissingComponents(block, anomalies) {
...
function checkStructuralRatios(item, anomalies) {
  ...
  if (fwRange && item.compositeFactors.formwork_ratio > 0) {
    ...
    if (ratio < fwRange.min * 0.5 || ratio > fwRange.max * 2) {
      anomalies.push({
  ...
  if (rbRange && item.compositeFactors.rebar_ratio > 0) {
    ...
    if (ratio < rbRange.min * 0.5 || ratio > rbRange.max * 2) {
      anomalies.push({
        type: "ratio_deviation",
        severity: "WARNING",
        sourceSheet: item.sourceSheet,
...
function checkDuplicateItems(items, anomalies) {
...
function checkPriceOutliers(materials, anomalies) {
...
function priceCategoryKey(name, unit) {
  ...
  for (const [pattern, key] of categories) {
    if (pattern.test(n)) return `${key}\xB7${u}`;
  }
  return null;
}
function convertToStagingRows(parsed) {
  ...
  for (const mat of parsed.materials) {
    ...
    const conf = mat.resolvedCode ? mat.matchConfidence : scoreConfidence(
      parsedData,
      ["code", "name", "unit"]
    );
    rows.push({
...
function resolveAhsBlockLinks(parsed) {
  ...
  for (const item of parsed.boqItems) {
    ...
    for (const blockIdx of linkedBlocks.ahsBlockIndices) {
      const list = directMap.get(blockIdx) ?? /* @__PURE__ */ new Set();
      list.add(item.code);
...
function findLikelyBoqCodesForAhsBlock(block, boqItems) {
  ...
  const isStrongEnough = best.shared >= 3 && best.score >= 0.32 && scoreGap >= 0.08 || best.shared >= 2 && best.score >= 0.5 && scoreGap >= 0.15;
  return isStrongEnough ? [best.code] : [];
}
function buildTitleMatchTokens(text) {
  const stripped = normalize(text).replace(/^\d+\s*(m1|m2|m3|kg|ls|bh|pcs|titik|set|unit)\b/, " ").replace(/\b(fc|slump|non|fly|ash|self|consolidating|concrete|scc)\b/g, " ").replace(/\s+/g, " ").trim();
  ...
  const tokens = stripped.split(" ").map((token) => token.trim()).filter((token) => token.length > 1 && !stopwords.has(token));
...
function classifyBoqItem(item) {
  ...
  const group = floor && !chapterLabel.toLowerCase().includes("lantai") ? `${chapterLabel} ${floor}` : chapterLabel;
  return { group, floor };
}
function extractFloorFromBoqContext(item) {
...
function groupBoqItems(items, aiClassifications) {
  ...
  const result = [];
  const sourceRowMapping = /* @__PURE__ */ new Map();
  const chapterCounters = /* @__PURE__ */ new Map();
  for (const [groupLabel, group] of groups) {
    ...
    const totalVolume = homogeneousUnit ? group.reduce((s, e) => s + e.item.volume, 0) : 1;
    const totalCost = {
    ...
    const sectionDetail = subLabels.length <= 5 ? subLabels.join(", ") : `${subLabels.slice(0, 4).join(", ")} (+${subLabels.length - 4} lainnya)`;
    result.push({
      code,
      label: groupLabel,
      unit,
...
function applyBoqGrouping(parsed, aiClassifications) {
...
var BOQ_WORK_TYPES = WORK_TYPE_ARCHETYPES.map((a) => a.type);
function getWorkTypeLabel(type, floor) {
  const arch = WORK_TYPE_ARCHETYPES.find((a) => a.type === type);
  return arch ? arch.label(floor) : `Pekerjaan Lainnya${floor ? ` ${floor}` : ""}`;
}
