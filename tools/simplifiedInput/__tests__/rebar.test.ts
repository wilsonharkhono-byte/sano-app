import { buildRebarComponent, REBAR_NAME_BY_DIAMETER, REBAR_COLUMNS } from '../rebar';

describe('rebar batang component builder', () => {
  it('maps the six file diameters to natural curated rebar names', () => {
    expect(REBAR_COLUMNS.map((c) => c.diameter)).toEqual([8, 10, 13, 16, 19, 22]);
    expect(REBAR_NAME_BY_DIAMETER[10]).toBe('Besi beton ulir 10 mm');
    expect(REBAR_NAME_BY_DIAMETER[8]).toBe('Besi beton polos 8 mm');
  });
  ...
  it('builds a batang component whose coefficient reproduces the lonjor count', () => {
    ...
    expect(c.materialName).toBe('Besi beton ulir 10 mm');
