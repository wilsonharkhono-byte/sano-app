...
export const CATEGORY_WORK: Record<string, WorkType> = {
...
export interface RequestedLine {
  ...
  status: string;
  created_at: string;
  /** material_request_headers.reviewed_at; the decision time when APPROVED or REJECTED. */
...
export function coverageByRow(
  ...
  const inGroup = (materialId: string | null) => { const g = materialGroupOf(input.catalog, materialId); return !!g && g.category === category && g.unit === unit; };
