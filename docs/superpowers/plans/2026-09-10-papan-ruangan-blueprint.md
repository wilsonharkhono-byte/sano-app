...
6. **Who may reassign: office roles and the reporter, as spec §9 says; being the current owner neither grants nor bars it.** An earlier draft of this plan read §9 as also excluding the current owner (an owner handing off their own overdue item). That exclusion was never implemented: `canEditAssignment` in `timelineModel.ts` and migration 099 both check only `is_office_role() OR reporter`, and their tests now assert exactly that rule. Whether to add an owner exclusion later is an open product decision recorded for the user; if it is adopted, it must land in TypeScript and SQL together with tests that refuse it, not as a comment. See D3 below.
...
| D3 | §9: "edit owner and due date (office roles and the reporter)" | Both `canEditAssignment` and migration 099 grant access to exactly the spec's two groups — office roles and the reporter. Being the current **owner** is neither a grant nor a bar: an office role or the reporter who is also the owner can still hand the item on. An explicit owner hand-off lockout was considered and is **not** implemented in release 1; the user may add it later. | The spec lists two groups, and owner-ness is not layered on top of them as a third test. Called out because it is an interpretation of a list, not a quotation, and because an owner-exclusion was drafted and then deliberately not shipped. |
