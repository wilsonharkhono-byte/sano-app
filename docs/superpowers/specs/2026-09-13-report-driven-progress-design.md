...
Settled while writing and rehearsing Plan B1 (`docs/superpowers/plans/2026-09-14-report-driven-progress-plan-b.md`). Where this section and §5.4, §6.2 or §11 disagree, this section wins. Two independent reviews (database and UI) followed the first build, and a follow-up review checked those fixes; every confirmed finding is folded in below.
...
**Evidence in B1.** Claim photos are storage paths under `progress/<project id>/`, at most 12 per line, replaced as a set on each save; the app asks for at least one photo when a row's figure rises. Verification copies a line's photos to `progress_photos` for a positive entry. The overview counts confirmed Blueprint lines per row since the claim's week start, from the latest revision of each report only. Two security-invoker views, `progress_claim_latest_verified` and `progress_entry_totals`, give the screens one row per BoQ item, so a long claim history never meets PostgREST's row cap; previews compute the quantity exactly as verification will, from the entry totals. The screens page those views and the weights past PostgREST's 1,000-row cap, and the verification preview reads the claim's BoQ rows fresh, because verification computes from the live planned volume. When verification still demands a reason the preview did not ask for, the panel opens the reason box on the row the refusal names and reloads its figures without losing what the estimator typed.

**Notifications.** `PROGRESS_CLAIM_SUBMITTED` goes to the project's estimators who neither submitted the claim nor filled one of its lines, since those cannot verify it (its admins when there are none, and its principals when there are neither, because notifications are readable only by project members and a principal can assign a verifier) and deeplinks `ProgressClaimVerify` (office Reports › Klaim). `PROGRESS_CLAIM_RETURNED` and `PROGRESS_CLAIM_VERIFIED` go to the submitter and deeplink `ProgressClaim` (the supervisor's Progres tab; Reports on an office navigator). The type list is 098's thirteen plus these three.

**Entry points.** Progres tab: Tambah progres opens the claim panel, and "Tambah progres untuk item ini" opens that row. Laporan: a Klaim Progres Mingguan card opens the same panel, without a fifth tab, so the tab row cannot overflow at 360 dp. Office Reports: a Klaim tab with a pending badge; the principal sees it read-only. Baseline: Bobot Tahapan. Principal home: Klaim Progres Minggu Ini, next to Progres vs Jadwal. Route params apply once per navigation and reload the claim panels and the Klaim badge. A refresh keeps the navigator mounted, so the user stays on their tab. A project switch unmounts it until the new project's data has loaded, which clears every screen's in-memory state (a half-built material request, cached envelopes) instead of carrying it into the other project, and a load that finishes after the user moved on is dropped. A notification for another project therefore switches first; the navigation waits in `workflows/pendingDeeplink.ts` and the router replays it once the navigator is back.

**Audit.** The "no progress in 7 days" anomaly counts claim lines created, lines edited while their claim is open, and claims submitted in the window. Returning or verifying a claim is never site activity on its own, and the check raises nothing when the tables cannot be read.

**Rehearsal.** `supabase/tests/progress_claims_rehearsal/run.sh` applies 001–102 to a disposable local `supabase/postgres` container, pastes 103 and 104 twice, and runs 155 behaviour checks as a supervisor, two estimators, an admin, the principal and an outsider, including every confirmed review finding.
